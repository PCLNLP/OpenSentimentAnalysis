...
from BIGCN_ABSA.model import Model
from BIGCN_ABSA.dataset import ABSADatesetReader, BucketIterator
from BIGCN_ABSA.eval import EvalEngine
from BIGCN_ABSA.utils.get_position import Vocab_post

class Instructor:
    def __init__(self, opt):
        self.opt = opt
        post_vocab = Vocab_post.load_vocab(os.path.join(opt.data_dir, opt.dataset, 'vocab_post.pkl'))
        absa_dataset = ABSADatesetReader(opt, post_vocab)
        ...
        self.net = Model(absa_dataset.embedding_matrix, common_adj, fre_embedding, post_vocab)
        ...
        self.grad_fn = ops.value_and_grad(self.forward_fn, None, ParameterTuple(self.net.trainable_params()), has_aux=True)

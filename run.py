...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default=Path('data/SenticBERT_ABSA_data'), type=Path)
    parser.add_argument('--dataset', default='rest16', type=str, help='twitter, restaurant, laptop')
    parser.add_argument('--ckpt', default=Path('checkpoints/ms_intergcn_bert_rest16_acc_0.9075_f1_0.7239.ckpt'),
                        type=str, help='The checkpoint used in eval mode.')
    parser.add_argument('--valset_ratio', default=0.1, type=float)
    parser.add_argument('--bert_tokenizer', default='bert-base-chinese', type=str, help='The tokenizer used in ABSA.')
    ...
    if opt.mode == 'train':
        ...
        ins = algo.Instructor(opt)
        ins.eval(str(opt.data_dir / opt.ckpt))
